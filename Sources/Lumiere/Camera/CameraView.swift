            ...
            case .authorized:
                CameraPreviewView(session: session.session)
                    ...
                    .overlay(alignment: .bottom) {
                        CameraHUD(recorder: session.recorder)
                    }
...
private struct CameraHUD: View {
    ...
    /// F-CFA-MVP-1 falsifier threshold (spec §19.2): p95 latency > 25 ms
    /// retracts the real-time claim. The HUD turns red above this value.
